...
- `pnpm test:e2e:playwright` runs the direct Playwright Test suite in `e2e/tests/`.
- `pnpm test:e2e:ui` opens Playwright UI mode for selecting, running, and inspecting tests interactively.
- `pnpm test:e2e:playwright:headed` runs the direct Playwright suite with a visible browser.
- `pnpm test:e2e:debug` opens the Playwright Inspector for the direct Playwright suite.
- Run one test with `pnpm exec playwright test e2e/tests/fairbets.spec.js -g "loads and shows"`.
